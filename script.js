...
let authListenerUnsubscribe = null; // Untuk membersihkan listener auth
...
function initializeFirebase() {
    try {
            ...
            authListenerUnsubscribe = onAuthStateChanged(auth, async (user) => {
                if (user) {
                    ...
                    try {
                        ...
                        message = `Gagal mengautentikasi pengguna. Pastikan fitur Anonymous Authentication diaktifkan di Firebase Console Anda (Authentication > Sign-in method). Detail: ${error.message}`;
                    }
                }
        ...
        transactionStatus = 'error';
        message = `Gagal menginisialisasi Firebase. Periksa koneksi atau konfigurasi Anda. Detail: ${e.message}`;
        renderApp(); // Tetap render untuk menampilkan error
    }
}
...
async function handleTopUp() {
    ...
    // Ini adalah bagian kritis: periksa apakah Firebase sudah terinisialisasi dan user terautentikasi
    if (!firebaseInitialized || !db || !auth || !currentUserId) {
        transactionStatus = 'error';
        message = 'Aplikasi tidak terhubung ke database atau pengguna tidak terautentikasi. Pastikan Firebase Authentication (Anonymous) dan Firestore Rules sudah benar. Coba muat ulang halaman.';
        renderApp();
        return;
    }

    const finalPrice = calculateFinalPrice();
    const appIdFromConfig = firebaseConfig.appId;

    try {
        // Coba tambahkan dokumen ke Firestore
        ...
        // Jika berhasil, lanjutkan ke WhatsApp
...
📪Metode : ${selectedPaymentMethod.name}
        ...
        transactionStatus = 'error';
        message = `Gagal menyimpan transaksi ke database. Ini mungkin karena Firestore Security Rules tidak mengizinkan penulisan. Pastikan aturan 'create' sudah benar. Detail: ${error.message}`;
        renderApp();
    }
}
